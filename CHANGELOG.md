...
### Fixed
- Fixed WebSocket protocol error "CONTINUATION frame without prior !FIN" when connecting to servers that don't support compression (e.g., OpenSensorHub/Jetty) by disabling `perMessageDeflate`
- Fixed double encoding of string messages - strings are now sent as-is instead of being JSON.stringify'd (e.g., "hello" no longer becomes "\"hello\"")
- Changed `node.log()` to `node.debug()` so debug messages don't appear when nodes/flows are disabled
